...
class MaleIdol extends Idol {
    // 부모 클래스와 property가 모두 동일하므로 constructor를 통해 super를 호출하는 것을 생략할 수 있다.
...
const country = new Country(
    ...
    [
        new IdolGroup(
            "ive",
            iveMembers.map(im => FemaleIdol.fromObject(im))
        ),
        new IdolGroup(
            "bts",
            btsMembers.map(bm => MaleIdol.fromObject(bm))
        )
    ]
);

console.log(country.getName, country.getIdolGroups[0].getName, country.getIdolGroups[0].getMembers.map(m => m.dance()));
console.log(country.getName, country.getIdolGroups[1].getName, country.getIdolGroups[1].getMembers.map(m => m.sing()));